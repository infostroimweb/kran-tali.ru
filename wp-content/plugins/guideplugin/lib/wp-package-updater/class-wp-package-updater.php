...
if ( ! class_exists( 'WP_Package_Updater' ) ) {
	...
	class WP_Package_Updater {
		...
		public function __construct(
			...
			if ( $this->use_license ) {
				...
				if ( 'Plugin' === $this->type ) {
					...
					add_filter( 'plugin_action_links_' . $this->package_id, array($this, 'print_license_toggle') );
				} elseif ( 'Theme' === $this->type ) {
		...
		public function print_license_toggle( $links ) {
			$toggle_license_link = array(
				'<a data-target="#'.$this->package_slug.'-license-container" class="plugin-license-toggle-button" style="cursor: pointer;">'.__('Manage license', 'wp-package-updater').'</a>',
			);
			return array_merge( $links, $toggle_license_link );
		}
