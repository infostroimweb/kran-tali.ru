...
class QSM_Install {
  ...
  $mlwQuizMasterNext->pluginHelper->register_quiz_setting( $field_array, 'quiz_options' );
    ...
    $field_array = array(
      ...
      'type' => 'editor',
      'default' => '%QUESTION%<br />Ответ: %USER_ANSWER%<br/>Correct Answer: %CORRECT_ANSWER%<br/>Comments Entered: %USER_COMMENTS%',
      'variables' => array(
        '%QUESTION%',        
        '%USER_ANSWER%',
        '%CORRECT_ANSWER%',
    ...
    global $mlwQuizMasterNext;
  	...
  	if ( get_option( 'mlw_quiz_master_version' ) != $data ) {
  		...
  		if($wpdb->get_var("SHOW COLUMNS FROM ".$table_name." LIKE 'question_answer_template'") != "question_answer_template")
  		{
  			...
  			$results = $wpdb->query( $sql );
  			$mlw_question_answer_default = "%QUESTION%<br /> Ответ: %USER_ANSWER%<br /> Correct Answer: %CORRECT_ANSWER%<br /> Comments Entered: %USER_COMMENTS%<br />";
  		...
  		if($wpdb->get_var("SHOW COLUMNS FROM ".$table_name." LIKE 'email_from_text'") != "email_from_text")
  		{
  			...
  			$results = $wpdb->query( $sql );
  			$update_sql = "UPDATE ".$table_name." SET email_from_text='Kran-tali'";
  			$results = $wpdb->query( $update_sql );
  		}
