...
function qmn_polar_display( $id, $question, $answers ) {
	...
	$step               = $mlwQuizMasterNext->pluginHelper->get_question_setting( $id, 'step' );
	$total_answer = count( $answers );
	$min = explode(" ", $answers[0][0]);
	$max = explode(" ", $answers[1][1]);
	?>
					...
					$('#slider-' + '<?php echo $id; ?>').slider({
						...
						<?php } ?>
							...
							change: function( event, ui ) {
								...
								$('.question-section-id-<?php echo $id; ?> .question-type-polar-s').find( ".polar-2" ).val(ui.value);
							...
							create: function( event, ui ) {
								...
								console.log(<?php echo $check_point / 2; ?>);
								$('.question-section-id-<?php echo $id; ?> .question-type-polar-s').find( ".polar-2" ).val(<?php echo $check_point / 2; ?>);
							}    
					});
					$('.question-section-id-<?php echo $id; ?> .question-type-polar-s').find('.qmn_polar').on('keyup', function () {
						$(this).val($(this).val().replace(/[^\d]/g,''));
						let sum = $(this).val();
						if (sum < <?php echo $answers[0][1]; ?>) sum = <?php echo $answers[0][1]; ?>;
						if (sum > <?php echo $answers[1][1]; ?>) sum = <?php echo $answers[1][1]; ?>;
						$('#slider-' + '<?php echo $id; ?>').slider("value", sum);
					});

					//$('.question-section-id-<?php echo $id; ?> .question-type-polar-s').find( ".polar-2" ).on('change', function(){

					//	$(this).val($(this).val().replace(/[^\d]/g,''));
					//	let sum = $(this).val();
					//	if (sum < <?php echo $answers[0][1]; ?>) sum = <?php echo $answers[0][1]; ?>;
					//	if (sum > <?php echo $answers[1][1]; ?>) sum = <?php echo $answers[1][1]; ?>;
					//	$('.question-section-id-<?php echo $id; ?> .question-type-polar-s').find('.qmn_polar').val(sum);
					//	$('#slider-' + '<?php echo $id; ?>').slider("value", sum);
					//})

					$('.question-section-id-<?php echo $id; ?> .question-type-polar-s').find( ".polar-2" ).on('keyup', function(){
						if(event.keyCode == 13){
							event.preventDefault();
						var change_val = $(this).val().replace(/[^\d\.\,]/g,''),
							change_val = change_val.replace(',','.');

						$(this).val(change_val);
							var sum = Number($(this).val()),
								min = Number(<?php echo $min[0]; ?>),
								max = Number(<?php echo $max[0]; ?>);
	...
	$input_text .= "<div class='slider-main-wrapper'><input type='text' class='qmn_polar $mlw_requireClass' id='question" . $id . "' name='question" . $id . "' />";
	$input_text .= '<div id="slider-' . $id . '"></div></div>';
	$unit = explode(' ', $answers[1][0]);
