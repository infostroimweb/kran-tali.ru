    ...
    <form action="" method="post" name="bulk_delete_form">
        ...
        <table class=widefat>
            ...
            <tr>
                ...
                <th><?php esc_html_e( 'Название опроса','quiz-master-next' ); ?></th>
				<?php
				$table_heading_displays = '';
				//$table_heading_displays .= '<th>' . esc_html__( 'Score', 'quiz-master-next' ) . '</th>';
				$table_heading_displays .= '<th>' . esc_html__( 'Время заполнения', 'quiz-master-next' ) . '</th>';
				$table_heading_displays .= '<th>' . esc_html__( 'Имя', 'quiz-master-next' ) . '</th>';
				...
				$table_heading_displays .= '<th>' . esc_html__( 'Телефон', 'quiz-master-next' ) . '</th>';
				//$table_heading_displays .= '<th>' . esc_html__( 'Заголовок страницы', 'quiz-master-next' ) . '</th>';
				$table_heading_displays .= '<th>' . esc_html__( 'Дата', 'quiz-master-next' ) . '</th>';
			...
			if ( $mlw_quiz_data ) {
				foreach ( $mlw_quiz_data as $mlw_quiz_info ) {
					...
					//print_r(unserialize($mlw_quiz_info->quiz_results));
					$results_contact = unserialize($mlw_quiz_info->quiz_results);
					$quiz_result_item_inner .= "<td><span style='font-size:16px;'>".$results_contact["contact"][ 0 ]["value"]."</span></td>";
					//$quiz_result_item_inner .= "<td><span style='font-size:16px;'>" . $mlw_quiz_info->business . "</span></td>";
					if($results_contact["contact"][ 2 ]["value"]) {
						$quiz_result_item_inner .= "<td><span style='font-size:16px;'>".$results_contact["contact"][ 2 ]["value"]."</span></td>";
					} else {
						$quiz_result_item_inner .= "<td></td>";
					}
					$quiz_result_item_inner .= "<td><span style='font-size:16px;'>".$results_contact["contact"][ 1 ]["value"]."</span></td>";
					/*if ( 0 == $mlw_quiz_info->user ) {
						$quiz_result_item_inner .= "<td><span style='font-size:16px;'>".$mlw_quiz_info->quiz_id."</span></td>";
					} else {
						$quiz_result_item_inner .= "<td><span style='font-size:16px;'>".$mlw_quiz_info->quiz_id."</span></td>";
					}*/
					$date        = date_i18n( get_option( 'date_format' ), strtotime( $mlw_quiz_info->time_taken ) );
					$time        = date( "d-m-y G:i", strtotime( $mlw_quiz_info->time_taken ) );
					$quiz_result_item_inner .= "<td><span style='font-size:16px;'><abbr>$time</abbr></span></td>";
